...
bool CFrameworkPackaging::get_fmi_sensor_view_config(osi3::SensorViewConfiguration& data)
{
    if (integer_vars[FMI_INTEGER_SENSORVIEW_CONFIG_SIZE_IDX] > 0)
    {
        ...
        normal_log("OSMP",
                   "SensorViewConfiguration: Got %08X %08X, reading from %p ...",
...
void CFrameworkPackaging::set_fmi_sensor_view_config_request(const osi3::SensorViewConfiguration& data)
{
    ...
    encode_pointer_to_integer(
        currentConfigRequestBuffer.data(), integer_vars[FMI_INTEGER_SENSORVIEW_CONFIG_REQUEST_BASEHI_IDX], integer_vars[FMI_INTEGER_SENSORVIEW_CONFIG_REQUEST_BASELO_IDX]);
    integer_vars[FMI_INTEGER_SENSORVIEW_CONFIG_REQUEST_SIZE_IDX] = (fmi2Integer)currentConfigRequestBuffer.length();
    normal_log("OSMP",
               "SensorViewConfiguration: Providing %08X %08X, writing from %p ...",
...
fmi2Status CFrameworkPackaging::doInit()
{
    ...
    for (int& boolean_var : boolean_vars)
    {
        ...
        fmi_verbose_log("OSMP", "doInit Boolean %d: %d", i, boolean_vars[i]);
        i++;
    }
 ...
 */
#include <model/profiles/init_profile.cpp>
            else string_var = "";

        fmi_verbose_log("OSMP", "doInit String %d: '%s'", i, string_vars[i].c_str());
        i++;
    }
...
fmi2Status CFrameworkPackaging::doStart(fmi2Boolean toleranceDefined, fmi2Real tolerance, fmi2Real startTime, fmi2Boolean stopTimeDefined, fmi2Real stopTime)
{
    DEBUGBREAK()
...
fmi2Status CFrameworkPackaging::doCalc(fmi2Real currentCommunicationPoint, fmi2Real communicationStepSize, fmi2Boolean noSetFMUStatePriorToCurrentPointfmi2Component)
{
    DEBUGBREAK()
...
void CFrameworkPackaging::doFree(){DEBUGBREAK()}

/*
 * Generic C++ Wrapper Code
 */

CFrameworkPackaging::CFrameworkPackaging(fmi2String theinstanceName,
                                         fmi2Type thefmuType,
    ...
    : instanceName(theinstanceName),
      ...
      sequence_of_strategies(profile, message_sink, alert_sink)
{
...
fmi2Status CFrameworkPackaging::SetDebugLogging(fmi2Boolean theloggingOn, size_t nCategories, const fmi2String categories[])
{
...
fmi2Component CFrameworkPackaging::Instantiate(fmi2String instanceName,
                                               fmi2Type fmuType,
                                               fmi2String fmuGUID,
...
{
    auto* myc = new CFrameworkPackaging(instanceName, fmuType, fmuGUID, fmuResourceLocation, functions, visible, loggingOn);
...
fmi2Status CFrameworkPackaging::SetupExperiment(fmi2Boolean toleranceDefined, fmi2Real tolerance, fmi2Real startTime, fmi2Boolean stopTimeDefined, fmi2Real stopTime)
{
...
fmi2Status CFrameworkPackaging::DoStep(fmi2Real currentCommunicationPoint, fmi2Real communicationStepSize, fmi2Boolean noSetFMUStatePriorToCurrentPointfmi2Component)
{
...
FMI2_Export fmi2Component fmi2Instantiate(fmi2String instanceName,
                                          ...
                                          fmi2Boolean loggingOn)
{
    return CFrameworkPackaging::Instantiate(instanceName, fmuType, fmuGUID, fmuResourceLocation, functions, visible, loggingOn);
}
...
FMI2_Export fmi2Status fmi2DoStep(fmi2Component c, fmi2Real currentCommunicationPoint, fmi2Real communicationStepSize, fmi2Boolean noSetFMUStatePriorToCurrentPointfmi2Component)
{
    auto* myc = (CFrameworkPackaging*)c;
