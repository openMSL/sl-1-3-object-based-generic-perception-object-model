...
#include "TransformationFunctions.hpp"
...
Vector3d TF::transform_to_local_coordinates(const Vector3d& input_coordinates, const Orientation3d& new_origin_rotation, const Vector3d& new_origin_translation) {
    ...
    double x_rel = delta_x * rot_mat(0,0) +
                   delta_y * rot_mat(0,1) +
                   delta_z * rot_mat(0,2);
    double y_rel = delta_x * rot_mat(1,0) +
                   delta_y * rot_mat(1,1) +
                   delta_z * rot_mat(1,2);
    double z_rel = delta_x * rot_mat(2,0) +
                   delta_y * rot_mat(2,1) +
                   delta_z * rot_mat(2,2);
...
Vector3d TF::transform_from_local_coordinates(const Vector3d& input_coordinates, const Orientation3d& new_origin_rotation, const Vector3d& new_origin_translation) {
    ...
    Eigen::Matrix3d rot_mat = calc_rotation_matrix_from_euler_angles(new_origin_rotation);
    double x_rel = input_coordinates.x() * rot_mat.transpose()(0,0) +
                   input_coordinates.y() * rot_mat.transpose()(0,1) +
                   input_coordinates.z() * rot_mat.transpose()(0,2);
    double y_rel = input_coordinates.x() * rot_mat.transpose()(1,0) +
                   input_coordinates.y() * rot_mat.transpose()(1,1) +
                   input_coordinates.z() * rot_mat.transpose()(1,2);
    double z_rel = input_coordinates.x() * rot_mat.transpose()(2,0) +
                   input_coordinates.y() * rot_mat.transpose()(2,1) +
                   input_coordinates.z() * rot_mat.transpose()(2,2);
...
Orientation3d TF::transform_orientation_from_world_to_sensor_coordinates(const Orientation3d &orientation_in_world_coordinates, const TF::EgoData &ego_data, const MountingPosition &mounting_pose) {
...
Orientation3d TF::transform_orientation_from_sensor_to_world_coordinates(const Orientation3d &orientation_in_sensor_coordinates, const TF::EgoData &ego_data, const MountingPosition &mounting_pose) {
...
Vector3d TF::transform_position_from_sensor_to_object_coordinates(const Vector3d &pcl_segment_position_in_sensor, const TF::EgoData &ego_data, const MountingPosition &mounting_pose, const BaseMoving &object_base) {
...
Vector3d TF::transform_position_from_object_to_sensor_coordinates(const Vector3d &pcl_segment_position_in_object, const TF::EgoData &ego_data, const MountingPosition &mounting_pose, const BaseMoving &object_base) {
...
double TF::projection_to_spherical_surface(std::vector<Spherical3d> &spherical_coordinates, double distance) {
    ...
    double processed_prev;
    double processed_next;
    double ready_to_add;
