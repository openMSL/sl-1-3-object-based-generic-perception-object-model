...
std::vector<GroundTruthObject> FrontEndStrategy::bring_ground_truth_objects_to_unified_format(const osi3::SensorView &input_sensor_view, const TransformationFunctions::EgoData& ego_data, const Profile &profile, const Alert &alert) {
    ...
    for (const auto &current_object : input_sensor_view.global_ground_truth().stationary_object()) {
        GroundTruthObject gt_object = FrontEndStrategy::get_stationary_object_from_ground_truth(current_object, ego_data, profile, alert);
    ...
    for (const auto &current_object : input_sensor_view.global_ground_truth().moving_object()) {
        if (current_object.id().value() != input_sensor_view.global_ground_truth().host_vehicle_id().value()) {
            GroundTruthObject gt_object = FrontEndStrategy::get_moving_object_from_ground_truth(current_object, ego_data, profile, alert);
            ground_truth_object_list.push_back(gt_object);
        }
    }
...
GroundTruthObject FrontEndStrategy::get_stationary_object_from_ground_truth(const osi3::StationaryObject &input_object, const TransformationFunctions::EgoData &ego_data, const Profile &profile, const Alert &alert) {
    ...
    //ego coordinates
    gt_object.base_ego_coord.mutable_position()->CopyFrom(TransformationFunctions::transform_to_local_coordinates(input_object.base().position(), ego_data.ego_base.orientation(), ego_data.ego_base.position()));
    gt_object.base_ego_coord.mutable_orientation()->CopyFrom(TransformationFunctions::calc_relative_orientation_to_local(input_object.base().orientation(), ego_data.ego_base.orientation()));

    //sensor coordinates
    osi3::MountingPosition mounting_pose = TransformationFunctions::get_sensor_mounting_pose(profile, alert);
    osi3::Vector3d position_sensor_coord = TransformationFunctions::transform_position_from_world_to_sensor_coordinates(input_object.base().position(), ego_data, mounting_pose);
    gt_object.base_sensor_coord.mutable_position()->CopyFrom(position_sensor_coord);
    gt_object.position_spherical_sensor_coord = TransformationFunctions::transform_cartesian_to_spherical(position_sensor_coord);

    osi3::Orientation3d orientation_sensor_coord = TransformationFunctions::transform_orientation_from_world_to_sensor_coordinates(input_object.base().orientation(), ego_data, mounting_pose);
    gt_object.base_sensor_coord.mutable_orientation()->CopyFrom(orientation_sensor_coord);

    return gt_object;
}

GroundTruthObject FrontEndStrategy::get_moving_object_from_ground_truth(const osi3::MovingObject &input_object, const TransformationFunctions::EgoData &ego_data, const Profile &profile, const Alert &alert) {
...
void FrontEndStrategy::write_vertices_to_logical_detections(osi3::SensorData &sensor_data, std::vector<GroundTruthObject> &ground_truth_object_list, const TransformationFunctions::EgoData& ego_data) {
    for(auto& current_object : ground_truth_object_list) {
        for(auto& current_vertex : current_object.bounding_box_vertices) {
            osi3::Vector3d current_vertex_word_coord = TransformationFunctions::transform_from_local_coordinates(current_vertex, current_object.osi_gt_object.base().orientation(), current_object.osi_gt_object.base().position());
            osi3::Vector3d current_vertex_ego_coord = TransformationFunctions::transform_position_from_world_to_ego_coordinates(current_vertex_word_coord, ego_data);
...
void FrontEndStrategy::write_visible_vertices_to_detections(osi3::SensorData &sensor_data, std::vector<GroundTruthObject> &ground_truth_object_list, const TransformationFunctions::EgoData& ego_data) {
