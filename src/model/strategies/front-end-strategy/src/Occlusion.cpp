                         ...
                         const Profile& profile)
{
    ...
    if (profile.sensor_view_configuration.radar_sensor_view_configuration().empty()  // lidar
        || (!profile.sensor_view_configuration.radar_sensor_view_configuration().empty() && ground_clearance < profile.sensor_parameters.radar_multipath_min_ground_clearance))
    {  // radar, but ground_clearance to low
...
ClipperLib::Paths calculate_clipped_polygons(std::vector<Vertex>& vertices_on_hull, std::vector<std::vector<Vertex>>& polygons_in_occlusion_process, float float_to_int_factor)
{
    ...
    {
        IntPoint current_point(static_cast<int>(current_vertex.vertex_on_plane.x() * float_to_int_factor),
                               static_cast<int>(current_vertex.vertex_on_plane.y() * float_to_int_factor));
              ...
              const Alert& alert)
{
    ...
    for (auto& current_fov_vertex : fov_vertices)
    {
        ...
        IntPoint current_point(static_cast<int>(current_vertex_plane.vertex_on_plane.x() * float_to_int_factor),
                               static_cast<int>(current_vertex_plane.vertex_on_plane.y() * float_to_int_factor));
        clipper[0] << current_point;
    }
    ...
    if (current_vertex.vertex3d.distance() == 0)
    {
        ...
        for (const auto& current_surface : current_gt_object.bounding_box_surfaces)
        {
            ...
            if (l_dot_N != 0)
            {
                ...
                Vector3d intersection_point_object_coord =
                    TF::transform_to_local_coordinates(intersection_point, current_gt_object.base_sensor_coord.orientation(), current_gt_object.base_sensor_coord.position());
...
{
    return (edge_end.vertex_on_plane.x() - edge_begin.vertex_on_plane.x()) * (point.vertex_on_plane.y() - edge_end.vertex_on_plane.y()) -
           (edge_end.vertex_on_plane.y() - edge_begin.vertex_on_plane.y()) * (point.vertex_on_plane.x() - edge_end.vertex_on_plane.x());
    // 0: collinear
    // positive: left
    // negative: right
}
...
{
    return (edge_end.vertex_on_plane.x() - edge_begin.vertex_on_plane.x()) * (edge_end.vertex_on_plane.x() - edge_begin.vertex_on_plane.x()) +
           (edge_end.vertex_on_plane.y() - edge_begin.vertex_on_plane.y()) * (edge_end.vertex_on_plane.y() - edge_begin.vertex_on_plane.y()) -
           (point.vertex_on_plane.x() - edge_begin.vertex_on_plane.x()) * (point.vertex_on_plane.x() - edge_begin.vertex_on_plane.x()) -
           (point.vertex_on_plane.y() - edge_begin.vertex_on_plane.y()) * (point.vertex_on_plane.y() - edge_begin.vertex_on_plane.y());
...
void graham_scan(std::vector<Vertex>& vertices_on_hull, const std::vector<Vertex>& all_vertices)
{
    ...
    for (auto vertex_itr = all_vertices.begin(); vertex_itr != all_vertices.end(); ++vertex_itr)
    {  // element at 0 is copied into ptr_subject_head at initialization
    ...
    {
        if (std::abs(vertex_processing.vertex_on_plane.x() - start_itr->vertex_on_plane.x()) < 0.00001 &&
            std::abs(vertex_processing.vertex_on_plane.y() - start_itr->vertex_on_plane.y()) < 0.00001)
        {
            continue;
        }
    ...
    for (auto& vertex_push_back : temp_container)
    {
        do
        {
            ...
            double position_flag = vertex_relative_to_line(vertices_on_hull.end()[-2], vertices_on_hull.end()[-1], vertex_push_back);
            if (position_flag < 0)
            {
            ...
            if (position_flag == 0)
            {
                double distance_flag = distance_comparison(vertices_on_hull.end()[-2], vertices_on_hull.end()[-1], vertex_push_back);
                if (distance_flag > 0)
                {
                    vertices_on_hull.pop_back();
                    continue;
