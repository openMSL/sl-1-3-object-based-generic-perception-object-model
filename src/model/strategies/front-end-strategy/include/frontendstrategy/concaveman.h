...
{
  public:
    bool operator()(const T& a, const T& b) { return (std::get<0>(a) < std::get<0>(b)); }
};

template <class T>
T orient2d(const std::array<T, 2>& p1, const std::array<T, 2>& p2, const std::array<T, 2>& p3)
{
...
template <class T>
bool intersects(const std::array<T, 2>& p1, const std::array<T, 2>& q1, const std::array<T, 2>& p2, const std::array<T, 2>& q2)
{

    auto res = (p1[0] != q2[0] || p1[1] != q2[1]) && (q1[0] != p2[0] || q1[1] != p2[1]) && (orient2d(p1, q1, p2) > 0) != (orient2d(p1, q1, q2) > 0) &&
...
template <class T>
T getSqDist(const std::array<T, 2>& p1, const std::array<T, 2>& p2)
{
...
template <class T>
T sqSegDist(const std::array<T, 2>& p, const std::array<T, 2>& p1, const std::array<T, 2>& p2)
{
...
class rtree
{
  public:
    ...
    void insert(data_type data, const bounds_type& bounds)
    {
        ...
        auto leaf = make_unique2<type>(best_child.get().data(), best_child.get().bounds());
...
struct Node
{
    ...
    Node(const point_type& p) : Node() { this->p = p; }
...
class CircularElement
{
  public:
    ...
    CircularElement<T>* prev() { return m_prev; }

    CircularElement<T>* next() { return m_next; }
...
#ifdef DEBUG_2
template <class T>
void snapshot(const std::array<T, 2>& a,
...
std::vector<std::array<T, 2>> concaveman(const std::vector<std::array<T, 2>>& points,
...
template <class T, int MAX_CHILDREN>
std::array<T, 2> findCandidate(const rtree<T, 2, MAX_CHILDREN, std::array<T, 2>>& tree,
                               ...
                               bool& ok)
{
    ...
    while (true)
    {
        ...
        while (!queue.empty() && std::get<1>(queue.top()).get().is_leaf())
        {
            ...
            if (-std::get<0>(item) < d0 && -std::get<0>(item) < d1 && noIntersections(b, p, segTree) && noIntersections(c, p, segTree))
            {
...
template <class T, int MAX_CHILDREN>
bool noIntersections(const std::array<T, 2>& a, const std::array<T, 2>& b, const rtree<T, 2, MAX_CHILDREN, typename CircularElement<Node<T>>::ptr_type>& segTree)
{
