...
void CsvOutputDetectedObjects::write_first_line_to_csv(const std::string& path)
{
    ...
    my_file << "timestamp_in_s, tracking_id, x_in_m, y_in_m, z_in_m, roll_in_deg, pitch_in_deg, yaw_in_deg, width_in_m, length_in_m, height_in_m, is_moving, existence_probability"
            << std::endl;
    my_file.close();
}
