                                                              ...
                                                              osi3::SensorData& sensor_data) const
{
    ...
    if (profile.data_extraction_parameters.detection_threshold_dB_stdv > 0.0)
    {
        std::normal_distribution<double> distribution(0.0,
                                                      profile.data_extraction_parameters.detection_threshold_dB_stdv);  // dist(mean, stddev), normal distribution with stddev in dB
        std::knuth_b generator(std::rand());                                                                            // rand used for Windows compatibility
        detection_threshold_dB += distribution(generator);
    }
    bool is_detected = mean_vertex_position_of_current_object.distance() < profile.data_extraction_parameters.max_range_in_m && power_equivalent_value_db > detection_threshold_dB;
...
double DataExtractionStrategy::calc_visible_area(const std::vector<Vector2d>& proj_vertices_current_obj, double mean_vertex_distance_of_current_object)
{
    ...
    if (proj_vertices_current_obj.size() > 2)
    {
        ...
        {
            projected_area_on_cylinder +=
                proj_vertices_current_obj.at(vertex_idx).x() * (proj_vertices_current_obj.at(vertex_idx + 1).y() - proj_vertices_current_obj.at(vertex_idx - 1).y());
        }
...
double DataExtractionStrategy::calculate_irradiation_gain(double azimuth_angle_rad, double elevation_angle_rad) const
{
    ...
    if (upper_bound_azimuth_index > 0 && upper_bound_azimuth_index < azimuth_pattern.size() && upper_bound_elevation_index > 0 &&
        upper_bound_elevation_index < elevation_pattern.size())
    {
        float elevation_interpolation =
            get_interpolation_factor(elevation_angle_deg, elevation_pattern.at(upper_bound_elevation_index - 1), elevation_pattern.at(upper_bound_elevation_index));
