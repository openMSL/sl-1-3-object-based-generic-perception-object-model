    ...
    if ((sensor_data.sensor_view_size() > 0) && sensor_data.sensor_view(0).has_global_ground_truth() && sensor_data.sensor_view(0).global_ground_truth().has_timestamp()) {
        ...
        for (const MovingObject& current_gt_object : sensor_data.sensor_view(0).global_ground_truth().moving_object()) {
            ...
            if (current_gt_object.has_base()) {
                ...
                get_pcl_segment_of_current_object(sensor_data.logical_detection_data(), data_of_current_time_step, current_gt_object.id().value());
                if (!data_of_current_time_step.pcl_segment_points_in_ego_coordinates.empty()) {
                    ...
                    if (profile.tracking_parameters.velocity_flag > 0) { // 0: Idealized velocity from GT
                        calculate_velocity_from_pcl(data_of_current_time_step, current_moving_object, object_contained_in_history, historical_object_no);
                    }
                }
                else {
                ...
                if (object_contained_in_history && history.sensor_data.back().moving_object(historical_object_no).has_base()) {
                    if (profile.tracking_parameters.tracking_flag == 0) { // 0: Idealized tracking
                        ...
                        continue_tracking_for_current_pcl_segment(
                            data_of_current_time_step, current_moving_object, current_gt_object, object_tracked_in_history, historical_object_no);
                    }
                } else {
...
void Tracking::get_pcl_segment_of_current_object(const LogicalDetectionData& logical_detection_data, Tracking::Data& data_of_current_time_step, uint64_t gt_object_id)
{
    ...
    switch (profile.tracking_parameters.dimension_and_position_flag) {
        ...
        case 2: // 2: Dimension from current point cloud segments with lower bounds, position as center of manipulated pcl segment
            eliminate_ground_clearance(
                ...
                &data_of_current_time_step.pcl_segment_position_in_object);
            calculate_dimension_with_lower_bounds(profile.tracking_parameters.minimum_object_dimension, data_of_current_time_step.pcl_segment_dimension,
                                              &data_of_current_time_step.pcl_segment_dimension,
                                              data_of_current_time_step.pcl_segment_position_in_object,
                                              &data_of_current_time_step.pcl_segment_position_in_object);
            current_moving_object->mutable_base()->mutable_dimension()->CopyFrom(data_of_current_time_step.pcl_segment_dimension);
        ...
        case 3: // 3: Maximum dimension of current and mean of historical point cloud segments, position as center of manipulated pcl segment
            eliminate_ground_clearance(
                ...
                data_of_current_time_step.pcl_segment_position_in_object,
                &data_of_current_time_step.pcl_segment_position_in_object);
            set_object_dimension_with_tracking(
                data_of_current_time_step.pcl_segment_dimension, &data_of_current_time_step.pcl_segment_dimension, object_contained_in_history, historical_object_no);
            current_moving_object->mutable_base()->mutable_dimension()->CopyFrom(data_of_current_time_step.pcl_segment_dimension);
...
void Tracking::eliminate_ground_clearance(const Dimension3d &current_dimension, const Dimension3d &current_gt_object_dimension, Dimension3d *new_dimension, Vector3d &current_position_in_object, Vector3d *new_position_in_object) {
...
[[maybe_unused]] void Tracking::limit_to_gt_height(Vector3d &pcl_segment_position_in_object, const Dimension3d &current_dimension, const Dimension3d &current_gt_object_dimension, Dimension3d *new_dimension) {
    new_dimension->CopyFrom(current_dimension);
...
void Tracking::calculate_dimension_with_lower_bounds(const Dimension3d &lower_bounds_dimension, const Dimension3d &current_pcl_segment_dimension, Dimension3d *new_pcl_segment_dimension,
                                                     const Vector3d &current_pcl_segment_position_in_object, Vector3d *new_pcl_segment_position_in_object) {
    if (lower_bounds_dimension.length() > current_pcl_segment_dimension.length()) {
        new_pcl_segment_position_in_object->set_x(current_pcl_segment_position_in_object.x()
                                                 - copysign(1.0, current_pcl_segment_position_in_object.x())
    ...
    if (lower_bounds_dimension.width() > current_pcl_segment_dimension.width()) {
        new_pcl_segment_position_in_object->set_y(current_pcl_segment_position_in_object.y()
                                                 - copysign(1.0, current_pcl_segment_position_in_object.y())
...
void Tracking::set_object_dimension_with_tracking(const Dimension3d& current_dimension,
                                                  ...
                                                  int historical_object_no) const {
...
void Tracking::calculate_velocity_from_pcl(Data& data_of_current_time_step, DetectedMovingObject* current_moving_object, bool object_contained_in_history, int historical_object_no)
{
...
void Tracking::calculate_velocity_as_derivation_of_position(Tracking::Data &data_of_current_time_step, DetectedMovingObject *current_moving_object, bool object_contained_in_history) const {
    Vector3d velocity;
    velocity.set_x(0);
    velocity.set_y(0);
    velocity.set_z(0);
...
void Tracking::calculate_orientation_from_history(Tracking::Data &data_of_current_time_step, DetectedMovingObject *current_moving_object, int historical_object_no) {
    ...
    current_moving_object->mutable_base()->mutable_orientation()->set_pitch(
            history.sensor_data.back().moving_object(historical_object_no).base().orientation().pitch()
            + history.sensor_data.back().moving_object(historical_object_no).base().orientation_rate().pitch() * data_of_current_time_step.delta_t);
    current_moving_object->mutable_base()->mutable_orientation()->set_yaw(
            history.sensor_data.back().moving_object(historical_object_no).base().orientation().yaw()
                                                         ...
                                                         int historical_object_no)
{
    ...
    if (object_tracked_in_history) {
        ...
        if (!data_of_current_time_step.pcl_segment_points_in_ego_coordinates.empty()) {
            ...
            current_moving_object->mutable_header()->set_existence_probability(
                    history.sensor_data.back().moving_object(historical_object_no).header().existence_probability() + profile.tracking_parameters.existence_probability_increment);
        ...
        } else { // Object currently not visible
            /// Reduce existence probability
            current_moving_object->mutable_header()->set_existence_probability(
                    history.sensor_data.back().moving_object(historical_object_no).header().existence_probability() - profile.tracking_parameters.existence_probability_decrement);
    ...
    } else { // Object wasn't tracked in the last time step
        if ((data_of_current_time_step.pcl_segment_points_in_ego_coordinates.size() > profile.tracking_parameters.min_detections_in_segment_for_tracking)
            ...
            if (!history.sensor_data.back().moving_object(historical_object_no).header().has_existence_probability()) {
                ...
                current_moving_object->mutable_header()->set_existence_probability(
                        history.sensor_data.back().moving_object(historical_object_no).header().existence_probability()
                        + profile.tracking_parameters.existence_probability_increment);
            } /// Start counting the age at the time the object tracking is being started (existence probability > threshold)
            if (current_moving_object->header().existence_probability() >= profile.tracking_parameters.existence_probability_threshold_for_tracking) {
            ...
            current_moving_object->mutable_header()->set_existence_probability(history.sensor_data.back().moving_object(
                    historical_object_no).header().existence_probability() - profile.tracking_parameters.existence_probability_decrement);
            data_of_current_time_step.object_is_moving.push_back(false);
        }
    }
}
...
void Tracking::start_tracking_for_current_pcl_segment(Data &data_of_current_time_step, DetectedMovingObject *current_moving_object, const MovingObject &current_gt_object) {
    double current_absolute_gt_velocity = sqrt(pow(current_gt_object.base().velocity().x(), 2) +
                                               pow(current_gt_object.base().velocity().y(), 2) +
                                               pow(current_gt_object.base().velocity().z(), 2));
    if((data_of_current_time_step.pcl_segment_points_in_ego_coordinates.size() > profile.tracking_parameters.min_detections_in_segment_for_tracking)
    || (!data_of_current_time_step.pcl_segment_points_in_ego_coordinates.empty() && current_absolute_gt_velocity > 0)) {
        current_moving_object->mutable_header()->set_age(0);
        current_moving_object->mutable_header()->set_existence_probability(profile.tracking_parameters.existence_probability_increment);
...
void Tracking::update_history(Tracking::Data &data_of_current_time_step, const SensorData & sensor_data) {
    history.sensor_data.emplace_back(data_of_current_time_step.sensor_data);
    history.sensor_data.back().add_sensor_view();
    history.sensor_data.back().mutable_sensor_view(0)->mutable_global_ground_truth()->mutable_moving_object()->CopyFrom(
        sensor_data.sensor_view(0).global_ground_truth().moving_object());
...
void Tracking::write_data_back_to_osi(SensorData & sensor_data, Data& data_of_current_time_step){
    for (const osi3::DetectedMovingObject& current_moving_object : data_of_current_time_step.sensor_data.moving_object()) {
        if (current_moving_object.header().existence_probability() >= profile.tracking_parameters.existence_probability_threshold_for_tracking) {
            DetectedMovingObject *added_output_moving_object = sensor_data.mutable_moving_object()->Add();
...
void Tracking::calculate_delta_t_of_current_time_step(Tracking::Data &data_of_current_time_step) {
    if (!history.sensor_data.empty() && history.sensor_data.back().has_timestamp()) {
        data_of_current_time_step.delta_t = static_cast<double>(data_of_current_time_step.sensor_data.timestamp().seconds()) + double(data_of_current_time_step.sensor_data.timestamp().nanos()) / 1000000000
                                            - (static_cast<double>(history.sensor_data.back().timestamp().seconds()) + double(history.sensor_data.back().timestamp().nanos()) / 1000000000);
    } else {
        ...
        if (history.sensor_data.back().sensor_view(0).global_ground_truth().moving_object_size() > 0) {
            ...
            for (int old_object_no = 0; old_object_no < history.sensor_data.back().moving_object_size(); old_object_no++) {
                ...
                if (current_moving_object->header().tracking_id().value() == history.sensor_data.back().moving_object(old_object_no).header().tracking_id().value()) {
                    ...
                    if (history.sensor_data.back().moving_object(old_object_no).header().existence_probability() >= profile.tracking_parameters.existence_probability_threshold_for_tracking) {
...
std::string Tracking::get_vehicle_type_string(const DetectedMovingObject_CandidateMovingObject *current_moving_object_candidate) {
    ...
    switch (current_moving_object_candidate->vehicle_classification().type()) {
        ...
        case MovingObject_VehicleClassification_Type_TYPE_WHEELCHAIR:
            ...
            break;
        case MovingObject_VehicleClassification_Type_TYPE_SEMITRACTOR:
            vehicle_type_string = "SEMITRACTOR";
            break;
    }
